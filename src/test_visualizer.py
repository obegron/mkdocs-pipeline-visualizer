...
def test_nav_default_structure_generation(plugin, mock_config):
    ...
    pipeline_versions = {
        'group1': {
            ...
            'pipeline2': [('2.0', 'path/to/pipeline2.md'), ('1.0', 'path/to/pipeline2_old.md')]
        }
    }
    task_versions = {
        'group2': {
...
def test_add_to_versions_no_version(plugin):
    ...
    plugin.add_to_versions(resource, new_file, "pipeline", pipeline_versions, task_versions)

    assert "" in pipeline_versions
    assert "no-version-pipeline" in pipeline_versions[""]
    assert pipeline_versions[""]["no-version-pipeline"] == [("", os.path.normpath('pipelines/no-version-pipeline.md'))]
...
def test_add_to_versions(plugin):
    ...
    assert pipeline_versions[""]["version1-pipeline"] == [("1.0.0", os.path.normpath('pipelines/version1-pipeline.md'))]
...
def test_add_to_versions_with_grouping_offset(plugin):
    ...
    assert pipeline_versions[os.path.normpath("group1/group2")]["grouped-pipeline"] == [("1.0.0", os.path.normpath("group1/group2/pipelines/grouped-pipeline.md"))]
...
def test_add_to_versions_with_invalid_grouping_offset(plugin):
    ...
    assert pipeline_versions[""]["grouped-pipeline"] == [("1.0.0", os.path.normpath("group1/group2/pipelines/grouped-pipeline.md"))]
